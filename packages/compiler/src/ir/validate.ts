...
export const LIB_FN_SIGS: Record<IrLibFn, { argTypes: (IrType | null)[]; result: IrType }> = {
  ...
  "fs.utimesSync": { argTypes: [STRING, F64, F64], result: VOID },
    ...
    switch (e.kind) {
      ...
      case "recordKeyGet": {
        ...
        const surfaces = (t: IrType): boolean =>
          ...
          // A union value passing AS-IS into the wider result union
          // (unionArmsEmbedIdentically — the backends' surface fast path).
          (t.kind === "union" &&
            e.type.kind === "union" &&
            unionArmsEmbedIdentically(
              unions.get(t.unionId)?.arms ?? [],
              unions.get(e.type.unionId)?.arms ?? [],
            )) ||
          e.type.kind === "dyn";
